<template>
    <div class="relative">
        ...
        :class="[
            show && width>639 
            ? 'absolute z-30 transiton ease-in-out delay-150 hover:translate-y-8 hover:scale-125 hover:bg-[#202020] duration-300'
            : ''
        ]">
            <div @mouseover="show = true" @mouseleave="show = false">
                <img class="aspect-video cursor-poiter" 
                :src="thumbnail || ''" 
                :class="
                show 
                ? 'transiton ease-in-out delay-150 rounded-l-lg' 
                : 'rounded-lg',
            ...
            <div class="flex mt-1.5">
                ...
                <div class="px-1.5 text-white mt-1">
                    ...
                    <div class="text-[14px] text-gray-300 font-extrabold flex gap-1 items-center cursor-pointer">{{ user.substring(0,30) }}
...
<script setup>
import { defineProps, toRef, ref, watch, onMounted } from 'vue';
...
const { title,
    ...
    image,
    videoUrl,
    thumbnail } = toRef(props)

let show = ref(false)
let showVideo = ref(false)
let video = ref(null)
let window=ref(document.documentElement.clientWidth)
